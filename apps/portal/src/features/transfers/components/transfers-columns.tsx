  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align='end'>
        ...
        {canRetry && (
          <DropdownMenuItem
            onClick={() => retry.mutate(payment.id, { onSuccess: () => toast.success('Retried'), onError: () => toast.error('Failed') })}
          >
            Retry
          </DropdownMenuItem>
        )}
        {canCancel && (
          <DropdownMenuItem
            ...
            onClick={() => cancel.mutate(payment.id, { onSuccess: () => toast.success('Cancelled'), onError: () => toast.error('Failed') })}
          >
            Cancel
          </DropdownMenuItem>
        )}
...
export const transfersColumns: ColumnDef<PaymentOrder>[] = [
  {
    accessorKey: 'id',
    header: 'ID',
    cell: ({ row }) => (
  ...
  },
  {
    ...
    cell: ({ row }) =>
      row.original.amount
        ? `${row.original.amount.amount} ${row.original.amount.asset}`
        : '—',
  },
  {
